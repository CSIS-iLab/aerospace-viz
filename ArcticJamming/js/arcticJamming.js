...
async function init() {
  ...
  var map = await makeMap({
    ...
    onEachFeature: {
      click: function click() {
        ...
        document.querySelector('.leaflet-popup-close-button').style.opacity =
          '1'
      },
  ...
  function setIconSize() {
    var zoomLevel = map.leaflet.getZoom()
  ...
  map.leaflet.on('zoomend', setIconSize)
  ...
  function makeCustomGeoJsonOptions(m) {
    var colorKeyWidget = m.widgets.find(function(w) {
      return w.type === 'color'
    })
    var formKeyWidget = m.widgets.find(function(w) {
      return w.type === 'form'
    })

    function filter(feature) {
      var activeFilters = m.filters.length ? m.filters : filters
    ...
    function onEachFeature(feature, layer) {
      handleFeatureEvents(feature, layer, m)
    }
    ...
    var backgroundOptions = {
      ...
      pointToLayer: function pointToLayerBackground(feature, latlng) {
        return feature.properties.icon.toLowerCase() === 'dot'
          ? styleCustomPoint(feature, latlng, m, colorKeyWidget)
          : stylePoint(feature, latlng, m)
      }
    }
    var foregroundOptions = {
      ...
      pointToLayer: function pointToLayerForeground(feature, latlng) {
        return stylePoint(feature, latlng, m)
      }
    }
    return [backgroundOptions, foregroundOptions]
  }

  function styleCustomPoint(feature, latlng, m, colorKeyWidget) {
  ...
  function formatCustomPopupContent(feature, map) {
    if (
      ...
      var sta = feature.properties.date_start.split('/').map(function(value) {
        return convertType(value)
      })
      var end = feature.properties.date_end.split('/').map(function(value) {
        return convertType(value)
      })
      var startDate = new Date(sta[2], sta[0] - 1, sta[1])
    ...
    if (!feature.properties.startDate && !feature.properties.endDate) return
  ...
  function handleSceneClick(e, m) {
    ...
    m.filters[0] = function(feature, layers) {
      var bool = false
